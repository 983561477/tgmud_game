...
// 获得一级技能的随机技能排序
var combat_skills = self_member.getRandomCombatSkills(0);
//print("combat_skills.length: " + combat_skills.length);
// 选取第一个技能
var combat_skill = combat_skills[0];
// 获取技能消耗的战斗cd
var combat_skill_consume_cd = combat_skill.getAttr("consume_cd");
// print("combat_skill_consume_cd: " + combat_skill_consume_cd);
// 获取当前的战斗cd
var now_combat_cd = self_member.getTimerValue("CD", "combat_cd");
// print("now_combat_cd: " + now_combat_cd);

if(now_combat_cd >= combat_skill_consume_cd){
	// 计算剩余战斗cd
	var residue_combat_cd = now_combat_cd - combat_skill_consume_cd;
	// print("residue_combat_cd: " + residue_combat_cd);
	// 赋值消耗后的战斗cd
	self_member.setTimerValue("CD", "combat_cd", residue_combat_cd);
	// 获取该技能的id
	var combat_skill_id = combat_skill.getAttr("id");
	// 获取对应技能的随机排序招式
